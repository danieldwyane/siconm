...
class REST
{
    ...
    public function get($params = array(), $format = NULL)
    {
        // if ($params)
        // {
        // 	//$uri .= '?'.(is_array($params) ? http_build_query($params) : $params);
    	...
    	return $this->_call_sagp('get', $params, $format);
    }


    // public function post($uri, $params = array(), $format = NULL)
    // {
    //     return $this->_call_sagp('post', $uri, $params, $format);
    // }

    public function post($params = array(), $format = NULL)
    {
        return $this->_call_sagp('post', $params, $format);
    }


    // public function put($uri, $params = array(), $format = NULL)
    // {
    //     return $this->_call_sagp('put', $uri, $params, $format);
        
    // }

    public function put($params = array(), $format = NULL)
    {
        return $this->_call_sagp('put', $params, $format);
    ...
    protected function _call($method, $uri, $params = array(), $format = NULL)
    {
		...
		//$this->http_header('content_type', $this->mime_type);
    ...
    protected function _call_sagp($method, $params = array(), $format = NULL)
    {

    	//echo $method.'-'.$params.'-'.$format;exit;
    	if ($format !== NULL)
		{
			$this->format($format);
		}
        ...
        $this->_ci->curl->create($this->rest_server);
